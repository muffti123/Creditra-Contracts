...
//! the v7 interest-accrual engine (`creditra_credit::accrual`) and its public
...
//!
//! # See also
//! - `creditra_credit::accrual::apply_accrual` — the v7 accrual chokepoint.
//! - `creditra_credit::accrual::accrue_batch` — the batched public entrypoint.
...
mod integration {
    ...
    // ── Test 4.11 — accrue_batch empty batch succeeds (0 ≤ 50) ──

    /// Edge case: empty borrower list must succeed (trivially within the
    /// ACCRUE_BATCH_MAX bound). Validates the lower-bound edge of the size
    /// check alongside the upper-bound test in 4.10.
    #[test]
    fn accrue_batch_empty_succeeds_no_revert() {
        let (env, contract_id, _admin, _token) = setup_with_token(0_i128);
        let client = CreditClient::new(&env, &contract_id);
    ...
    // ── Test 4.12 — draw_credit on frozen line → CreditLineFrozen (46) ──

    /// `CreditLineFrozen` (46) is the compliance-hold freeze distinct from
    /// `BorrowerFrozen` / `DrawsFrozen`. Materialization via `draw_credit`
    /// (which invokes apply_accrual at its head) must encode #46.
    #[test]
    fn draw_head_accrual_line_frozen_emits_code_46() {
        let (env, contract_id, admin, _token) = setup_with_token(10_000_i128);
        let client = CreditClient::new(&env, &contract_id);
        let borrower = Address::generate(&env);
        ...
        assert!(
            err_str.contains("#46"),
            "expected CreditLineFrozen (#46), got: {:?}",
            err_str
        );
    }
}
